...
class GuiApi:
    ...
    def __init__(
        ...
        self._websock_interface.register_handler(
            _messages.GuiCloseModalRequestMessage, self._handle_close_modal_request
        )
        self._websock_interface.register_handler(
    ...
    async def _handle_close_modal_request(
        ...
        """Handle a client request to close a modal.

        We do this server-side so modal closure properly removes all contained GUI
        elements and broadcasts the closure to all connected clients.
        """
        del client_id
    ...
    def configure_theme(
        ...
        *,
        ...
        Args:
            ...
            titlebar_dark_mode_checkbox_uuid: Optional UUID of a checkbox handle. When set,
                a dark mode toggle is shown in the titlebar that drives this checkbox.
    ...
    def add_modal(
        ...
        *,
        ...
        Args:
            ...
            order: Optional ordering, smallest values will be displayed first.
            save_choice: Optional stable key for persisting a user's acknowledgement choice
                in the web client (e.g. via localStorage). If set, the client may auto-dismiss
                the modal on reload when the choice has been saved.
            size: Optional modal size hint for the client UI (e.g., "sm", "md", "lg", "xl", "full").
